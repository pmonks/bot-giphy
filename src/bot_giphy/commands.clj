...
(defn- status!
  ...
  (let [now           (tm/now)
        ...
        message       (str "<messageML>"
                           "<b>Giphy bot status as at " (u/date-as-string now) ":</b>"
                           "<p><table>"
...
(defn- config!
  ...
  (let [now     (tm/now)
        message (str "<messageML>"
                     "<b>Giphy bot config as at " (u/date-as-string now) ":</b>"
...
(defn- logs!
  ...
  [stream-id _]
  (let [tmp-zip-file (java.io.File/createTempFile "bot-giphy-logs-" ".zip")
        log-files    (cfg/log-files)]
    (u/zip-files! tmp-zip-file log-files)
    (sym/send-message! cnxn/symphony-connection stream-id
                                                (str "<messageML><b>Giphy bot logs as at " (u/now-as-string) ":</b></messageML>")
                                                nil
                                                tmp-zip-file)
