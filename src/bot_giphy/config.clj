...
(def git-url
  (str "https://github.com/pmonks/bot-giphy/tree/" git-revision))
...
(defn reload!
  ...
  []
  (log/debug "Reloading bot-giphy configuration...")
  (mnt/stop)
  (mnt/start)
  (log/debug "bot-giphy configuration reloaded."))
...
(defn set-log-level!
  "Sets the log level (which can be a string or a keyword) of the bot, for the given package (a String, defaults to 'bot-giphy', use 'ROOT' for the root logger)."
  ([level] (set-log-level! "bot-giphy" level))
